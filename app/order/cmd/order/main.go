...
func newApp(logger *slog.Logger, gs *grpc.Server, hs *http.Server, _ *bizorder.Sweeper, _ *bizorder.Streams) *kratos.App {
	return kratos.New(
		kratos.ID(id),
		kratos.Name(Name),
...
func main() {
	...
	app, cleanup, err := wireApp(&bc.Server, &bc.Data, &bc.Client, &bc.Auth, logger)
	if err != nil {
		panic(err)
	}
	defer cleanup()
